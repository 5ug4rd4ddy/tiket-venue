...
import string
from . import db
from .models import Ticket, Addon, Order, SiteSetting, PaymentMethod, User, PromoCode, Gate, Partner, SpecialDate
import csv
from io import StringIO
from flask import make_response
from datetime import datetime, timedelta
from sqlalchemy import func, or_
from .utils import send_invoice_email, send_eticket_email
import threading
    ...
    try:
        ...
        for key, qty in counts.items():
            if qty > 0:
                ...
                if ticket:
                    # price = ticket.price_adult if variant == 'adult' else ticket.price_child
                    price = ticket.get_price(date_status, variant)
                    name = f"{ticket.name} ({'Dewasa' if variant == 'adult' else 'Anak'})"
                    subtotal = price * qty
        ...
        return render_template('checkout.html', summary=summary, cities=cities_list)
...
def process_payment():
    try:
        ...
        details_obj = {
            ...
            'group': summary.get('group_details'),
            'promo': {
                'code': promo_code,
        ...
        new_order = Order(
            ...
            total_price=final_total, # Use discounted total
            ...
            payment_status='pending', # Default pending for all, including card
        ...
        try:
            ...
            print(f"Failed to start invoice email thread: {e}")
        
        # Clear session
        session.pop('checkout_summary', None)
        
        return jsonify({'status': 'success', 'order_id': new_order.id, 'order_uuid': new_order.uuid})
...
def admin_download_invoice(order_id):
    ...
    # Tickets
    for item in details.get('tickets', []):
        data_items.append([
            f"Tiket - {item['name']}",
            str(item['quantity']),
            f"Rp {item['price']:,}",
...
def admin_settings():
    ...
    if request.method == 'POST':
        ...
        # Payment Settings
        settings.payment_timeout_minutes = int(request.form.get('payment_timeout_minutes', 60))
...
@main.route('/ticket/add', methods=['POST'])
def add_ticket():
    if not session.get('logged_in'): return redirect(url_for('main.login'))
    name = request.form.get('name')
    price_adult = int(request.form.get('price_adult'))
    price_child = int(request.form.get('price_child'))
    
    # Dynamic Pricing
    paw = request.form.get('price_adult_weekend')
    pcw = request.form.get('price_child_weekend')
    pah = request.form.get('price_adult_highseason')
    pch = request.form.get('price_child_highseason')
    
    price_adult_weekend = int(paw) if paw and paw.strip() else None
    price_child_weekend = int(pcw) if pcw and pcw.strip() else None
    price_adult_highseason = int(pah) if pah and pah.strip() else None
    price_child_highseason = int(pch) if pch and pch.strip() else None
    ...
    db.session.add(Ticket(
        ...
        price_adult_highseason=price_adult_highseason,
        price_child_highseason=price_child_highseason,
        slug=slug, 
        is_active=is_active,
        category=category
    ))
...
@main.route('/ticket/edit/<int:id>', methods=['POST'])
def edit_ticket(id):
    if not session.get('logged_in'): return redirect(url_for('main.login'))
    ticket = Ticket.query.get_or_404(id)
    
    ticket.name = request.form.get('name')
    ticket.price_adult = int(request.form.get('price_adult'))
    ticket.price_child = int(request.form.get('price_child'))
    ...
    ticket.price_adult_weekend = int(paw) if paw and paw.strip() else None
    ticket.price_child_weekend = int(pcw) if pcw and pcw.strip() else None
    ticket.price_adult_highseason = int(pah) if pah and pah.strip() else None
    ticket.price_child_highseason = int(pch) if pch and pch.strip() else None
    
    ticket.description = request.form.get('desc')
    ticket.is_active = request.form.get('is_active') == 'on'
    ticket.category = request.form.get('category', 'personal')
...
def add_addon():
    ...
    db.session.add(Addon(name=name, description=desc, price=price, slug=slug, is_active=is_active))
    db.session.commit()
...
def edit_addon(id):
    ...
    addon.category = request.form.get('category', 'personal')
...
@main.route('/payment/add', methods=['POST'])
def add_payment():
    if not session.get('logged_in'): return redirect(url_for('main.login'))
...
@main.route('/payment/delete/<int:id>', methods=['POST'])
