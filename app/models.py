...
class Ticket(db.Model):
    ...
    price_adult = db.Column(db.Integer, nullable=False, default=0)
    price_child = db.Column(db.Integer, nullable=False, default=0)
    ...
    def get_price(self, date_type, variant):
        """
        Get price based on date_type (regular, weekend, high_season) and variant (adult, child).
        Implements inheritance: High Season -> Weekend -> Regular.
        """
        if variant == 'adult':
            base = self.price_adult
            weekend = self.price_adult_weekend
            high = self.price_adult_highseason
        else:
            base = self.price_child
